...
`StoreQueue.scala` 实现了XiangShan处理器的store队列，它是内存子系统的关键组件，负责处理store指令。StoreQueue管理store指令从发射到最终写入内存的全过程，包括store转发、内存一致性维护和非对齐访问处理等功能。

### 1.1 核心功能

- 接收和处理来自后端的store指令
- store-load转发，支持部分转发和精确的地址对比
- 管理store指令的提交和写回
- 处理MMIO (Memory Mapped I/O) 和非缓存 (Non-cacheable) store指令
- 支持非对齐访问和跨页store指令处理
- 向SBuffer传递已提交的store数据
- 支持向量store指令的特殊处理
...
1.  **数据前递**  ：实现高效的store-load转发机制，确保数据依赖的正确性
2.  **非对齐store指令**  ：支持处理非对齐的store访问，包括跨16字节边界和跨页访问
3.  **向量Store指令**  ：支持处理RISC-V向量扩展中的向量store指令
4.  **CMO指令**  ：实现缓存维护操作，用于管理缓存内容
5.  **CBO指令**  ：支持缓存块操作，特别是CBO.ZERO清零操作
6.  **MMIO与NonCacheable Store指令**  ：通过UnCache接口处理设备访问和非缓存store
7.  **store指令提交与SBuffer写入**  ：采用提前提交策略，高效地将store数据写入SBuffer
...
4. 处理特殊的store类型（如MMIO、非缓存、向量store等）
...
```mermaid
flowchart TB
    BE[后端重命名/调度] --> |发射store指令| SQ[store队列 StoreQueue]
    SQ --> |store-load转发| LQ[load队列]
    ROB[重排序缓冲区] --> |提交store指令| SQ
    SQ --> |写回已提交数据| SB[store缓冲区 SBuffer]
...
4.  **StoreExceptionBuffer**  ：store异常信息
...
这些数据类型定义了store队列中各种数据结构的格式和操作方式。

## 4. 内部数据结构与状态

StoreQueue内部维护了多种数据结构和状态标志，用于跟踪每条store指令的生命周期和状态：

### 4.1 主要数据数组

```scala
// 微操作数据，保存每条store指令的详细信息
...
StoreQueue使用多个状态标志数组跟踪store指令的生命周期和状态：
...
// 地址相关标志
val unaligned = RegInit(VecInit(List.fill(StoreQueueSize)(false.B)))  // 非对齐store
...
// 向量store相关标志
val isVec = RegInit(VecInit(List.fill(StoreQueueSize)(false.B)))  // 向量store指令
val vecLastFlow = RegInit(VecInit(List.fill(StoreQueueSize)(false.B)))  // 向量store的最后流
val vecMbCommit = RegInit(VecInit(List.fill(StoreQueueSize)(false.B)))  // 向量store从合并缓冲区提交

// 异常相关标志
val hasException = RegInit(VecInit(List.fill(StoreQueueSize)(false.B)))  // store有异常
val waitStoreS2 = RegInit(VecInit(List.fill(StoreQueueSize)(false.B)))  // 等待storeS2阶段结果
```

这些标志的组合使得StoreQueue能够准确跟踪每条store指令的状态和类型，从而正确处理各种复杂情况。
...
这些状态机确保特殊类型的store操作能够正确完成，同时避免与正常store指令的处理冲突。

## 5. store队列处理流程

StoreQueue处理store指令的整个生命周期，从入队到最终写入内存。以下是详细的处理流程：

### 5.1 指令入队

store指令从后端调度器发射后首先进入StoreQueue：
...
入队过程中，StoreQueue为每个有效的入队请求分配store队列条目，初始化所有状态标志和微操作信息，同时处理分支重定向可能导致的取消情况。
...
### 5.4 将提交的store写入SBuffer

当store指令被标记为已提交且地址和数据都有效后，store数据将写入数据缓冲区，然后发送到SBuffer：
...
这个过程完成了store指令的主要功能：将数据写入内存层次结构。
...
这些特殊处理机制确保MMIO和非缓存store能够正确完成，同时维护程序的执行顺序。

### 5.6 处理分支预测错误和异常

当发生分支预测错误时，StoreQueue需要撤销受影响的store指令：
...
这个机制确保错误预测路径上的store指令不会影响程序状态，是保证推测执行正确性的关键部分。

## 6. store-load转发机制

store-load转发是StoreQueue的核心功能之一，它确保后续的load指令能够从尚未写入缓存的store数据中读取正确的值。XiangShan的store-load转发设计在满足正确性的同时考虑了高性能需求。
...
转发的第一步是确定哪些store指令的地址与load指令匹配：
...
地址匹配通过CAM (Content Addressable Memory) 操作实现。SQAddrModule内部会比较store和load指令的地址，判断是否匹配。匹配分为精确匹配（地址完全相等且掩码相交）和行匹配（缓存行地址相等且store指令有行标志）。

### 6.3 转发数据生成

在匹配到store指令后，SQDataModule负责生成转发数据：
...
转发数据生成使用并行优先级逻辑，快速找到最相关的匹配（优先考虑较新的store指令），并生成对应的转发数据。
...
当无法进行转发时，StoreQueue需要提供依赖指示，告知LoadUnit需要replay：
...
这些依赖指示帮助LoadUnit决定何时重新执行load指令，是保证数据依赖正确性的关键机制。
...
内存映射I/O (MMIO) 和非缓存 (Non-cacheable) store是特殊类型的store操作，需要通过UnCache接口直接访问设备或内存，不使用缓存机制。XiangShan处理器中，StoreQueue负责这些特殊store指令的处理。

### 7.1 MMIO和非缓存store的识别

在地址生成阶段，store指令会被标记为MMIO或非缓存访问：
...
MMIOstore使用专用的状态机处理，只有当指令到达ROB头部并确认提交时才会执行：
...
非缓存(NC)store也使用专用的状态机处理，但与MMIO不同，它们在ROB提交后直接处理，不需要到达ROB头部：
...
非缓存store允许一定程度的并行执行，但仍然保证了程序顺序的正确性。

### 7.4 UnCache通道复用

MMIO和非缓存store共享同一个UnCache通道，但具有不同的优先级：
...
向量store的异常处理稍有不同，需要通过专门的反馈通道接收异常信息。MMIO访问可能在UnCache响应阶段检测到硬件错误。
...
1.  **检测异常**  ：在store地址计算阶段检测异常
2.  **跟踪异常**  ：将带有异常标记的store指令正常流过store队列
3.  **更新StoreExceptionBuffer**  ：将异常信息写入专用的异常缓冲区
4.  **提供精确异常信息**  ：当ROB处理异常时，提供精确的虚拟地址和异常信息
5.  **特殊情况处理**  ：处理MMIO硬件错误和向量store异常

### 8.5 向量store异常的特殊处理

向量store指令可能涉及多个元素，需要特殊的异常处理机制：
...
StoreQueue提供了丰富的性能监控和调试功能，帮助开发者理解和优化store系统性能。
...
StoreQueue对store-load转发过程提供了特殊的性能监控：
...
这些统计信息有助于理解和优化store-load转发机制。

### 9.3 队列状态输出

StoreQueue提供多个状态输出接口，帮助处理器其他部分监控store队列状态：
...
为了更好地理解StoreQueue的工作机制，下面通过两个具体实例分析store指令的完整处理流程，包括普通store指令和MMIOstore指令的处理。

### 11.1 普通store指令处理实例

我们以一条普通的32位字store指令为例，追踪其在StoreQueue中的完整生命周期。
...
store地址从地址生成单元(AGU)写入StoreQueue:
...
```
// ROB信号
io.rob.scommit(0) = true          // 提交store指令0
io.rob.scommit(1) = false         // 无提交store指令1
...
### 11.2 MMIOstore指令处理实例

MMIOstore指令需要特殊处理，通过UnCache渠道直接访问设备。
...
入队过程与普通store类似，此处简化。关键区别在于地址写入阶段:
...
MMIOstore指令必须等到到达ROB头部并成为暂挂store指令时才能执行:

```
io.rob.pendingst = true      // 存在暂挂store指令
...
通过这两个实例，我们可以看到StoreQueue如何处理不同类型的store指令，如何维护其状态的转换，以及如何与处理器中的其他组件（如ROB、SBuffer和UnCache接口）协同工作。
